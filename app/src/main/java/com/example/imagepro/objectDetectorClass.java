...
public class  objectDetectorClass {
        ...
        for (int i=0;i<10;i++){
            ...
            if(score_value>0.5){
                ...
                if (bitmap1 == null || bitmap1.getWidth() == 0 || bitmap1.getHeight() == 0) {
                    Log.e("BitmapError", "Bitmap has invalid dimensions: " + cropped.cols() + "x" + cropped.rows());
                    return mat_image;
                }
                Utils.matToBitmap(cropped,bitmap1);
                Log.e("BitmapError", "Invalid bitmap dimensions: " + Classification_input_size+ "x" + Classification_input_size);
                Bitmap scaledBitmap1=Bitmap.createScaledBitmap(bitmap1,Classification_input_size,Classification_input_size,false);
                if (scaledBitmap1.getWidth() == 0 || scaledBitmap1.getHeight() == 0) {
                    Log.e("BitmapError", "Scaled bitmap has invalid dimensions");
                    return mat_image;
                }
                ...
                Log.d("ObjectionDetectionClass","output_class_value "+output_class_value[0][0]);
    ...
    private String get_alphabets(float sig_v) {
        ...
        if (sig_v >=0.5 && sig_v < 1.5) {
            ...
            val = "?"; // Optional: handles values of sig_v outside the specified ranges
        }
        return val;
    }
